				...
				[]patchOperation{
					{
						...
						Value: map[string]interface{}{"name": "tvsi-vault-agent", "image": "vault:1.2.3", "command": []interface{}{"sh", "-c", "cat \u003c\u003cEOF \u003e vault-agent-config.hcl\npid_file = \"/home/vault/pidfile\"\n\nauto_auth {\n  method \"kubernetes\" {\n    mount_path = \"auth/kubernetes\"\n    config = {\n      role = \"test-app-1\"\n      token_path = \"/var/run/secrets/talend/vault-sidecar-injector/serviceaccount/token\"\n    }\n  }\n\n  sink \"file\" {\n    config = {\n      path = \"/home/vault/.vault-token\"\n    }\n  }\n}\nEOF\nworkload_is_job=\"false\"\nif [ $workload_is_job = \"true\" ]; then\n  docker-entrypoint.sh agent -config=vault-agent-config.hcl -log-level=info &\n  while true; do\n    if [ -f \"/home/vault/vault-sidecars-signal-terminate\" ]; then\n      echo \"=> exit (signal received)\"\n      export VAULT_TOKEN=$(cat /home/vault/.vault-token);\n      vault token revoke -self;\n      exit 0\n    fi\n    sleep 2\n  done\nelse\n  docker-entrypoint.sh agent -config=vault-agent-config.hcl -log-level=info\nfi\n"}, "env": []interface{}{map[string]interface{}{"name": "SKIP_SETCAP", "value": "true"}, map[string]interface{}{"name": "VAULT_ADDR", "value": "https://vault:8200"}}, "resources": map[string]interface{}{}, "volumeMounts": []interface{}{map[string]interface{}{"name": "vault-token", "mountPath": "/home/vault"}, map[string]interface{}{"name": "app-1-sa-token-m7vf4", "readOnly": true, "mountPath": "/var/run/secrets/talend/vault-sidecar-injector/serviceaccount"}}, "lifecycle": map[string]interface{}{"preStop": map[string]interface{}{"exec": map[string]interface{}{"command": []interface{}{"sh", "-c", "export VAULT_TOKEN=$(cat /home/vault/.vault-token); vault token revoke -self;\n"}}}}, "imagePullPolicy": "IfNotPresent"},
					},
					{
						Op: jsonPatchOpAdd, Path: jsonPathContainers + "/1",
						Value: map[string]interface{}{"name": "tvsi-consul-template", "image": "hashicorp/consul-template:0.22.0-alpine", "command": []interface{}{"sh", "-c", "cat \u003c\u003cEOF \u003e /consul-template/config/consul-template-config.hcl\nvault {\n  vault_agent_token_file = \"/home/vault/.vault-token\"\n  renew_token = false\n  retry {\n    attempts = 10\n    backoff = \"2s\"\n    max_backoff = \"60s\"\n  }\n  ssl {\n    enabled = true\n    verify = true\n  }\n}\n\ntemplate {\n    destination = \"/opt/talend/secrets/secrets.properties\"\n    contents = \u003c\u003cEOH\n    {{ with secret \"secret/test-app-1/test-app-1-svc\" }}{{ range \\$k, \\$v := .Data }}\n{{ \\$k }}={{ \\$v }}\n{{ end }}{{ end }}\n    EOH\n    command = \"\"\n    wait {\n    min = \"1s\"\n    max = \"2s\"\n    }\n}\n\nEOF\nworkload_is_job=\"false\"\nif [ $workload_is_job = \"true\" ]; then\n  docker-entrypoint.sh -config=/consul-template/config/consul-template-config.hcl -log-level=info &\n  while true; do\n    if [ -f \"/home/vault/vault-sidecars-signal-terminate\" ]; then\n      echo \"=> exit (signal received)\"\n      exit 0\n    fi\n    sleep 2\n  done\nelse\n  docker-entrypoint.sh -config=/consul-template/config/consul-template-config.hcl -log-level=info\nfi\n"}, "env": []interface{}{map[string]interface{}{"name": "VAULT_ADDR", "value": "https://vault:8200"}}, "resources": map[string]interface{}{}, "volumeMounts": []interface{}{map[string]interface{}{"name": "vault-token", "mountPath": "/home/vault"}, map[string]interface{}{"name": "secrets", "mountPath": "/opt/talend/secrets"}}, "imagePullPolicy": "IfNotPresent"},
					},
					{
